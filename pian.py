...
saran : spam jan ragu anjing
_______________________________________________
{m} {k} ♡{m} {p} Author :{p} ALPIXN-CEPMEK   |
{m} {k} ♡{m} {p} Github :{p} https://github.com/pian-hackers
'''
def wangsaf():
	os.system('clear'); print(banner)
	try:
		global target
		target = int(input(f'{m} [{h} ?{m} ]{p} Target Number 08×××× :{k} '))
	except ValueError:
	...
	try:
		while True:
			...
			if "success" in main.text:
				print(f'{m} [{p} ✓{m} ]{p} BERHASIL KIRIM VIRUS OTP WHATSAPP {h}{target}');s(3);true += 1
			else:
				print(f'{m} [{p} x{m} ]{p} GAGAL KIRIM VIRUS OTP WHATSAPP {a}{target}');s(3);false += 1
			...
			if "WA OTP dikirim" in main.text:
				...
				print(f'{m} [{p} x{m} ]{p} GAGAL KIRIM VIRUS OTP WHATSAPP To {a}{target}');s(3);false += 1
...
def semes():
	...
	try:
		while True:
			...
			if "success" in main.text:
				print(f'{m} [{p} ✓{m} ]{p} BERHASIL KIRIM VIRUS OTP SMS {h}+62{target}');s(3);true += 1
			else:
				print(f'{m} [{p} x{m} ]{p} GAGAL KIRIM VIRUS OTP SMS {a}+62{target}');s(3);false += 1
...
def start():
	os.system('clear'); print(banner); print(f'''{p} [{k} 1{p} ]{p} Spam Virus Via Whatsapp\n=================================]
{p} [{k} 2{p} ]{p} Spam Virus Via Sms
=================================]	{p}
 [{k} 3{p} ]{p} Spam Virus Via Whatsapp v1
=================================] {p}
 [{k} 4{p} ]{p} Spam Virus Via Sms v1''')
 
	sponge = input(f'\n{m} [{h} ?{m} ]{p} Pilih nomor :{h} ')
	if sponge == '1' or sponge == '01':
		wangsaf()
	elif sponge == '2' or sponge == '02':
		semes()
	if sponge == '3' or sponge == '03':
		wangsaff()
	elif sponge == '4' or sponge == '04':
		semess()	
	else:
		exit(f'{m} [{p} !{m} ]{p} Pilih Yang Bener Tolol...')
